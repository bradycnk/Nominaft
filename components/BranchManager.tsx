...
const BranchManager: React.FC = () => {
  ...
  const fetchBranches = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('sucursales').select('*').order('es_principal', { ascending: false });
    if (!error && data) {
      setBranches(data);
      const principal = data.find(b => b.es_principal);
      if (principal && principal.logo_url) setLogoUrl(principal.logo_url);
    }
    setLoading(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Si se marca como principal, desactivar las otras (Lógica de negocio: solo una principal)
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Logo oficial que aparecerá en los recibos de pago.</p>
          </div>
        </div>
        <button 
          onClick={() => logoInputRef.current?.click()}
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <div>
            <h2 className="text-xl font-bold text-slate-800">Sucursales y Sedes</h2>
            <p className="text-xs text-slate-500 font-medium">Control de entidades fiscales farmacéuticas</p>
          </div>
          <button 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50 text-slate-400 text-[10px] uppercase font-black tracking-widest">
              <tr>
                <th className="px-6 py-4">Sede / RIF</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                <tr><td colSpan={5} className="p-10 text-center text-slate-400">Cargando sedes...</td></tr>
              ) : branches.length === 0 ? (
                <tr><td colSpan={5} className="p-10 text-center text-slate-300 font-medium">No hay sucursales registradas</td></tr>
              ) : branches.map(branch => (
                <tr key={branch.id} className={`hover:bg-slate-50/50 transition-colors ${branch.es_principal ? 'bg-emerald-50/30' : ''}`}>
                  <td className="px-6 py-4">
                    <div className="text-sm font-bold text-slate-800">{branch.nombre_id}</div>
                    <div className="text-[10px] font-mono text-slate-400">RIF: {branch.rif || 'Sin Registrar'}</div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-center">
                    {branch.es_principal ? (
                      <span className="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-[9px] font-black uppercase tracking-widest border border-emerald-200">Principal</span>
                    ) : (
                      <span className="px-3 py-1 bg-slate-100 text-slate-400 rounded-full text-[9px] font-black uppercase tracking-widest border border-slate-200">Sucursal</span>
                    )}
                  </td>
                  <td className="px-6 py-4 text-center">
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg animate-in zoom-in duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Nombre / ID de Sucursal</label>
                  <input required className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none" value={formData.nombre_id} onChange={e => setFormData({...formData, nombre_id: e.target.value})} placeholder="Ej: Farmacia Principal" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">RIF Fiscal</label>
                  <input required className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none" value={formData.rif} onChange={e => setFormData({...formData, rif: e.target.value})} placeholder="J-12345678-9" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Dirección Fiscal Completa</label>
                <textarea required className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none h-24" value={formData.direccion} onChange={e => setFormData({...formData, direccion: e.target.value})} placeholder="Avenida Principal, Edificio Farma..." />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
              ...
              <div className="flex items-center gap-2 py-2">
                ...
                />
                <label htmlFor="es_principal" className="text-sm font-bold text-slate-700">Designar como Sede Principal / Fiscal</label>
              </div>
